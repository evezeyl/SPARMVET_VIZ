...
- [ ] **BP-SMOKE-1** `[sonnet/medium]`: Functional smoke pass in the running app (qa persona): (a) escape-hatch save round-trips; (b) help panel resolves `__doc__`; (c) a manifest **saved/exported from BLUEPRINT loads cleanly in HOME** (Save/Export → HOME round-trip, with `comment:` keys preserved).
