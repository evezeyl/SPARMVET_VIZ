...
**Last Updated:** 2026-05-09 (BP-AGENT-FLAG-1 ✅: persona flag + config block + cascade landed in 8 templates, bootloader, and `rules_persona_feature_flags.md`) by @dasharch
...
- [x] **BP-AGENT-FLAG-1** `[haiku/low]`: Added `blueprint_agent_enabled` flag to all 8 persona templates (Group D, cascade `blueprint_enabled: false` → `blueprint_agent_enabled: false`). False for the 6 scientist/demo templates, true for developer + qa. `blueprint_agent:` config block added to developer (`backend: claude_cli`) and qa (`backend: disabled` for deterministic CI). Cascade wired into `bootloader._load_persona_config()` Group D (docstring updated). `rules_persona_feature_flags.md` Group D + Full Flag Matrix + Cascade Enforcement §5 + Misconfiguration table + Files Governed updated. All 8 templates pass `scripts/validate_persona_config.py --all`. ADR-076 §6. ✅ 2026-05-09
