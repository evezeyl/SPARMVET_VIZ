...
**Last Updated:** 2026-05-09 (DIAG-CORE-1 ✅: ADR-078 Phase B landed — `DeploymentError` dataclass + helpers, PersonaValidator retrofit, server.py + CLI updated. ADR-079 placeholder + Phase C task block + 6 runtime-error tasks added.) by @dasharch
...
- [x] **AUDIT-NOTIF-UTIL-MOVE** `[haiku/low]`: `app/handlers/notification_utils.py` has zero `@render.*`/`@reactive.*` decorators (verified 2026-05-09). It is a pure utility — violates the Two-Category Law spirit by living in `handlers/`. Move to `app/modules/notification_utils.py`, update all import paths, update `@deps` blocks in callers.
...
- [x] **AUDIT-ADR076-MVP** `[opus/medium]`: Define a 1-page MVP-1 scope for ADR-076 (BLUEPRINT AI Agent Helper) before implementation starts. MVP-1 criteria: (1) `DisabledAdapter` + `ClaudeCliAdapter` only; (2) `get_available_actions` + `get_available_components` + `get_field_contract` tools only (no `propose_manifest_diff` yet); (3) chat panel renders responses; (4) no `BP-AGENT-REPORT-1` session bundle. Write the scope definition as a `## ADR-076 MVP-1` subsection in `architecture_decisions.md` or as a `.claude/design/adr076_mvp.md`. This prevents the feature from becoming a 4–6 week sink. ✅ 2026-05-09 — scoped at `.claude/design/adr076_mvp.md`; ADR-076 status line points at it; MVP-1 = 8 tasks (~1 week) vs full ADR (4–6 weeks); promotion criteria for MVP-2 defined.
...
- [x] **DIAG-CONNECTOR-1** `[sonnet/medium]`: Retrofit `libs/connector/` — `FilesystemConnector` (path validation), `IridaConnector` (auth missing, endpoint unreachable), `BioBlendConnector` (Galaxy auth). Critical for IRIDA/Galaxy deployments. **Decision needed:** move `deployment_error.py` to `libs/utils/` so connectors can import it without the cross-lib violation, OR copy the dataclass shape into `libs/utils/errors.py`. ADR-078 Phase C + ADR-011 cross-lib rule. ✅ 2026-05-09 — Option A chosen: DeploymentError moved to libs/utils/; app/modules/deployment_error.py is now a re-export shim.
- [x] **DIAG-MANIFEST-1** `[sonnet/medium]`: Retrofit manifest contract / structural validation at *load time* (NOT the assembler — that is ADR-079 territory). Covers: missing `analysis_groups`, malformed `data_schemas`, missing `!include` targets, malformed YAML in master manifest. Likely lives in `libs/utils/config_loader.py` or a new `libs/utils/manifest_preflight.py`. ADR-078 Phase C. ✅ 2026-05-09 — implemented in `libs/utils/src/utils/config_loader.py`: 3-stage validation (file/YAML parse → !include failures → structural checks), all emitting `DeploymentError` via `exit_if_errors`.
- [x] **DIAG-CATALOG-1** `[sonnet/high]`: Author `docs/troubleshooting/index.qmd` — one row per known `DeploymentError` with permalink slug. Each error's `reference:` field optionally points at the catalog entry by anchor (e.g. `docs/troubleshooting/#cascade-d-blueprint-agent`). The catalog becomes the operator's first stop on encountering an unfamiliar error. ADR-078 Phase C deliverable. ✅ 2026-05-09 — 31 errors cataloged across 5 groups (Bootloader, PersonaValidator, SidebarValidator, Connectors, ConfigManager); quick-reference table + per-error anchor sections; wired into `docs/_quarto.yml` Chapter 1.
- [x] **DIAG-CLI-1** `[haiku/low]`: Audit remaining CLI scripts (`assets/scripts/*.py`, `scripts/install_libs.sh` Python helpers) for failure modes that should emit `DeploymentError`-formatted output for consistency. ADR-078 Phase C tail. ✅ 2026-05-09 — identified 4 candidates: create_test_deployment, create_manifest, debug_apply_manifest_standards, build_dep_graph (all currently use bare sys.exit/print). Retrofitting deferred to future task.

### Runtime Error Discipline (ADR-079, placeholder)

**Why separate from ADR-078:** runtime errors raised mid-pipeline (during ingestion, wrangling, assembly, plot rendering) have a different audience (the analyst, not the operator), a different fix surface (data cleaning / manifest edits, not config edits), and a different render path (Shiny notification / plot card overlay, not stderr). Reusing the `DeploymentError` shape conceptually but with audience-aware in-app rendering.

- [ ] **DIAG-RUNTIME-ADR** `[opus/high]`: Author ADR-079 properly. Open questions in the ADR-079 placeholder section of `architecture_decisions.md`: separate class vs extend `DeploymentError`; render path per category; how the user discovers *what* in their data caused the failure (sample rows, offending values, schema diff); whether errors are captured in the export bundle audit trail. **Trigger:** when Phase C is largely complete, OR when the first runtime-error pain point becomes blocking, whichever comes first.
...
- [ ] **DIAG-RUNTIME-WRANGLER-1** `[deferred until DIAG-RUNTIME-ADR]`: Retrofit `libs/transformer/src/transformer/data_wrangler.py` — action-name not registered (suggest closest match), action arg validation against `ui_schema`.
- [ ] **DIAG-RUNTIME-VIZFACTORY-1** `[deferred until DIAG-RUNTIME-ADR]`: Retrofit `libs/viz_factory/src/viz_factory/viz_factory.py` — component not registered, missing required aesthetic, plotnine render exceptions surfaced with the offending layer name and aesthetic dict.
- [ ] **DIAG-RUNTIME-T3APPLY-1** `[deferred until DIAG-RUNTIME-ADR]`: T3 Apply path failures in `app/handlers/audit_stack.py` — downstream invalidation, comment-gate violations, recipe-shape errors.
- [ ] **DIAG-RUNTIME-BLUEPRINT-1** `[deferred until DIAG-RUNTIME-ADR]`: Manifest fragment validation failures during BLUEPRINT IDE editing in `app/handlers/blueprint_handlers.py`.
...
- [x] **BP-AGENT-FLAG-1** `[haiku/low]`: Added `blueprint_agent_enabled` flag to all 8 persona templates. False for the 6 scientist/demo templates, true for developer + qa. `blueprint_agent:` config block added to developer (`backend: claude_cli`) and qa (`backend: disabled`). **Cascade is fatal, not silent (ADR-077):** `_FATAL_CASCADE_GATES` in `persona_validator.py` + Rule 7 raises a startup error if `blueprint_agent_enabled=True` with `blueprint_enabled=False`. Same treatment retroactively applied to `manifest_edit_enabled` (ADR-075). Bootloader does NOT silently suppress these flags. `rules_persona_feature_flags.md` updated (Group D, Full Flag Matrix, Cascade Enforcement §4–5, Misconfiguration table, Files Governed) — soft vs fatal cascade distinction documented. All 8 templates pass; synthetic contradiction produces 2 fatal errors. ADR-076 §6 + ADR-077. ✅ 2026-05-09
