...
- [ ] **BP-PLOT-LEGACY-REMOVE-1** `[sonnet/high]`: **Clean break (ADR-083 §7a).** Remove from `normalise_plot_spec`: factory_id expansion, flat-aes promotion, `_FACTORY_GEOMS`, `bar_logic` y-check, `heatmap_logic` color→fill remap. Replace with hard helpful `PipelineError` (factory_id/flat-aes → "use explicit geom layer + `mapping:`; run migrate_plot_specs.py"). Update `test_plot_config_resolver.py`: drop the ~9 factory_id expansion tests, add legacy-error tests. **Gate (must pass before merge):** `grep -rn "factory_id" config/ assets/ app/ libs/ tests/` returns only this error code + historical ADR text. Depends BP-PLOT-COMMIT-1 + BP-PLOT-MIGRATE-1 + BP-PLOT-DOCS-1.
...
- [ ] **BP-COMMENTS-1** `[sonnet/medium]`: Free-text **intent `comment` field** on every node/step/group/plot in the BLUEPRINT form (wrangling nodes already have one — extend to all). Written as a `comment:` data key (preserved by ingest, no new dep). Input MUST have a hover tooltip nudging good-comment practice (e.g. "Write your intent — *why*, not what"). Q4 (revised 2026-05-20), ADR-082.
- [ ] **BP-GROUPS-1** `[sonnet/high]`: Group/plot sidebar inventory — persistent list with create/delete/assign affordances (MVP). **Build the group/plot CRUD decoupled from the sidebar UI** so the v2 TubeMap context menu (Q6-C) is a pure addition with no refactor. Q6, ADR-082.
...
- [ ] **BP-SMOKE-1** `[sonnet/medium]`: Functional smoke pass in the running app (qa persona): (a) escape-hatch save round-trips; (b) help panel resolves `__doc__`; (c) a manifest **saved/exported from BLUEPRINT loads cleanly in HOME** (Save/Export → HOME round-trip, with `comment:` keys preserved); (d) **BP-FORMS-UNIFY-1 live check** — clicking "Add Node" creates a stub and opens the rich `ui_schema` form on the right; filling it + Apply commits the node (no annotation modal). Note: full BLUEPRINT smoke is new test-infra (existing suite covers HOME only); the `#fb_op` filter tests flake under full-suite load — run filter tests in isolation.
