...
#REVIEW (should we instead rename the usermetadata except the minimal required columns that should not be in double ! by eg using `um_` prefix for user metadata ?) 
...
(um: user metadata) -> ` um_sanitization.py` :  sanitization function, can automatically handle column name conflics. It should also verify the format of metadata 
